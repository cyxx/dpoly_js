var decoder = {
	...
	decodeHelper1 : function( bits, len ) {
	...
	decode : function( data ) {
		...
		while ( this.m_size > 0 ) {
			if ( !this.nextBit( ) ) {
				this.m_code = 1;
				if ( !this.nextBit( ) ) {
					this.decodeHelper1( 3, 0 );
				} else {
					this.decodeHelper2( 8 );
				}
			} else {
				var code = this.readBits( 2 );
				switch ( code ) {
				case 3:
					this.decodeHelper1( 8, 8 );
					break;
				case 2:
					this.m_code = this.readBits( 8 );
					this.decodeHelper2( 12 );
					break;
				default:
					this.m_code = code + 2;
					this.decodeHelper2( code + 9 );
					break;
				}
			}
		}
